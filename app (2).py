...
# Version header
# v1.5.4-mah-only-for-displayed + lot-verify:
# - MAH capture (used in validity) restricted to suspect products that are displayed (Celix-matched).
# - Adds comment: 'Verify Lot No with Celix-Lot No List' whenever a real Lot number is present (alphanumeric, not nullFlavor).
# - Retains v1.5.2 fixes and logic.
def _get_password():
 ...
 if status == "launched_by_strength":
  if isinstance(payload, dict) and payload:
   if strength_mg is not None:
    return payload.get(strength_mg) if isinstance(list(payload.keys())[0], str) else payload.get(strength_mg)
  ...
  for idx, uploaded_file in enumerate(uploaded_files, start=1):
   ...
   for drug in root.findall('.//hl7:substanceAdministration', ns):
    ...
    if drug_id in suspect_ids:
     ...
     # NOTE: We will add MAH to case_mah_names only inside the Celix-matched block
     ...
     if matched_company_prod:
      ...
      lot_clean = ''
      # Treat HL7 nullFlavor (UNK/ASKU/NI/MSK) as empty
