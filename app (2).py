...
# ---------------------------------------------------------------------------------------------------------
# v1.10.2 - per-product event-wise listedness display, password removed, no debug prints
...
with tab1:
    ...
    if uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files, start=1):
            ...
            case_drug_dates_display: List[Tuple[str, Optional[float], Optional[date], Optional[date]]] = []  # (prod, strength, start, end)
                    ...
                    if matched_company_prod:
                        ...
                        if not has_any_patient_detail:
                            ...
                            if status in ("yet", "awaited"):
                                ...
                                frd_raw_obj_local = parse_date_obj(global_dates_local["FRD_raw"]) if global_dates_local["FRD_raw"] else None
                                lrd_raw_obj_local = parse_date_obj(global_dates_local["LRD_raw"]) if global_dates_local["LRD_raw"] else None
                                if launch_dt is not None:
                                    if frd_raw_obj_local and frd_raw_obj_local < launch_dt:
                                        exposure_reasons.append("FRD")
                                    if lrd_raw_obj_local and lrd_raw_obj_local < launch_dt:
            ...
            # --- GLOBAL FRD/LRD/TD for Report Date (unchanged) ---
            ...
            # ---------------------- Validity assessment (unchanged core logic) ----------------------
            ...
            try:
                ...
                is_non_valid_case = isinstance(validity_value, str) and str(validity_value).startswith("Non-Valid")
            ...
            all_rows_display.append({
                ...
                'Event Details': event_details_combined_display,  # NO listedness fragments here
                'Listedness': ('' if is_non_valid_case else event_wise_listedness_display),  # Event-wise; blank if Non-Valid
...
with tab2:
    ...
    if all_rows_display:
        ...
        edited_df = st.data_editor(
            ...
            disabled=df_display.columns  # disable all columns -> read-only
        )
