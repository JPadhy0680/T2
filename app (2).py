...
# Use literal emoji characters to avoid surrogate escape issues
st.title("📊🧠 E2B_R3 XML Triage Application 🛠️ 🚀")

# Version header
# v1.5.5-report-date-frd-lrd-td:
# - Adds detection of FRD/LRD in reactions: availabilityTime/@value is LRD, nearest preceding effectiveTime/low is FRD.
# - Renames 'Transmission Date' column to 'Report Date' and displays FRD, LRD, TD.
...
# NEW: Strictly product-scoped MAH (no root/global fallback)
...
with tab1:
    ...
    if uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files, start=1):
            ...
            case_event_dates = []
            # NEW: MAHs only for displayed products
            case_displayed_mahs = []
            ...
            for drug in root.findall('.//hl7:substanceAdministration', ns):
                ...
                if drug_id in suspect_ids:
                    ...
                    # MAH strictly from the same drug element
                    ...
                    if matched_company_prod:
                        ...
                        # Show MAH for this displayed product + collect for validity
                        if mah_name_clean:
            ...
            # Case-level placeholders for FRD/LRD (first detected within any reaction)
            ...
            for reaction in root.findall('.//hl7:observation', ns):
                ...
                if code_elem is not None and code_elem.attrib.get('displayName') == 'reaction':
                    ...
                    # Existing event start/end parsing (unchanged)
                    ...
                    # ---- FRD/LRD detection (availabilityTime value as LRD, nearest preceding effectiveTime/low as FRD) ----
                    FRD_disp = LRD_disp = ''
                    FRD_obj = LRD_obj = None
                    try:
                        children = list(reaction)  # ordered children of this <observation>
                        avail_idx = next((i for i, ch in enumerate(children) if ch.tag.endswith('availabilityTime')), None)
                        if avail_idx is not None:
                            avail_node = children[avail_idx]
                            avail_val = avail_node.attrib.get('value', '') or ''
                            if avail_val:
                                LRD_disp = clean_value(format_date(avail_val))
                                LRD_obj = parse_date_obj(avail_val)
                                # Search backwards for nearest preceding effectiveTime/low
                                FRD_candidate = None
                                for j in range(avail_idx - 1, -1, -1):
                                    ch = children[j]
                                    if ch.tag.endswith('effectiveTime'):
                                        low = ch.find('hl7:low', ns)
                                        if low is not None and ('value' in low.attrib):
                                            FRD_candidate = low.attrib.get('value', '') or ''
                                            break
                                if FRD_candidate:
                                    FRD_disp = clean_value(format_date(FRD_candidate))
                                    FRD_obj = parse_date_obj(FRD_candidate)
                                # Save first FRD/LRD encountered at case level
            ...
            has_celix_suspect = bool(case_products_norm)  # suspects that matched Celix product list (displayed)
            ...
            # 3) MAH shows non-Celix -> Non-company product (ONLY for displayed products)
            ...
            if validity_reason is None:
                for prod, strength_mg, sdt, edt in case_drug_dates_display:
                    ...
                    if status in ("yet", "awaited"):
                        ...
                        break
            # 5) Dates before launch -> Drug exposure prior to Launch — ONLY displayed drugs
            if validity_reason is None:
                launch_dates = []
                for prod, strength_mg, sdt, edt in case_drug_dates_display:
                ...
                if validity_reason is None and launch_dates:
